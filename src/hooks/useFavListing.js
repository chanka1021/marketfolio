...
export const useFavListing = () => {
    ...
    const favListing = async (data) => {
        ...
        try{
            const response = await axios.post(`http://18.193.116.80:2005/user/addfav`, {
                headers: {
    ...
    const unfavListing = async (data) => {
        ...
        setError(null);
        try{
            const response = await axios.post(`http://18.193.116.80:2005/user/removefav`, {
                headers: {
                    'authorization': `Bearer ${user.token}`,
                },
                body: JSON.stringify(data),
    ...
    const fetchFavListing = async (id) => {
        try {
            const data = await axios.get(`http://18.193.116.80:2005/listing/fav/${id}`, {
                headers: {
                    'authorization': `Bearer ${user.token}`,
                },
            });
